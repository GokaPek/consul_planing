...
public interface SecurityAccountApi {
    ...
    TokenResponse signUp(
            @RequestBody(description = "Данные для регистрации нового пользователя", required = true)
    ...
    TokenResponse signIn(
            @RequestBody(description = "Данные для авторизации пользователя", required = true)
